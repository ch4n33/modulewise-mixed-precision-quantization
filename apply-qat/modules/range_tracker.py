...
class EMAActivationRangeTracker(RangeTracker):
  ...
  def forward(self, tensor):
    ...
    with torch.no_grad():
      if self.min is None:
        ...
        self.min = self.min * self.decay + tensor.min() * (1 - self.decay)
        self.max = self.max * self.decay + tensor.max() * (1 - self.decay)
...
class HistogramRangeTracker(RangeTracker):
  def __init__(self, coverage=0.99):
    # this tracker is deprecated
    raise NotImplementedError("HistogramRangeTracker is deprecated")
